...
def get_denseaspp_model(n_class=1):
    model = DenseASPP(class_num=n_class)
    return model

def test():
    device = torch.device('cuda')  # cuda:0
    inputs = torch.rand(3, 256, 256).unsqueeze(0).to(device)  # 不需要Batch
    print(inputs.shape)

    net = get_denseaspp_model().to(device)
    res = net(inputs)
