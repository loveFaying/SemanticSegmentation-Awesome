...
def get_r2unet_model():
    model = R2U_Net()
    return model
def get_r2attunet_model():
    model = R2AttU_Net()
    return model
def get_unet_plus_model():
    model = NestedUNet()
    return model
...
def test():
    ...
    r2unet = get_r2unet_model().to(device)
    r2attunet = get_r2attunet_model().to(device)
    unet_plus = get_unet_plus_model().to(device)
    res1 = r2unet(inputs)
    res2 = r2attunet(inputs)
    res3 = unet_plus(inputs)
    print('res1 shape:', res1.shape)
    print('res2 shape:', res2.shape)
    print('res3 shape:', res3.shape)

    num_params = 0
    for param in r2unet.parameters():
        num_params += param.numel()
    print("The number of parameters : %.3f M" % (num_params/1e6))

    num_params = 0
    for param in r2attunet.parameters():
        num_params += param.numel()
    print("The number of parameters : %.3f M" % (num_params/1e6))

    num_params = 0
    for param in unet_plus.parameters():
        num_params += param.numel()
    print("The number of parameters : %.3f M" % (num_params/1e6))

    # from thop import profile, clever_format
    # flops, params = profile(r2unet, inputs=(inputs,))
    # macs, params = clever_format([flops, params], "%.3f")
    # print('r2unet flops:', macs)
    # print('r2unet params:', params)

    # flops, params = profile(r2attunet, inputs=(inputs,))
    # macs, params = clever_format([flops, params], "%.3f")
    # print('r2attunet flops:', macs)
    # print('r2attunet params:', params)

    # flops, params = profile(unet_plus, inputs=(inputs,))
    # macs, params = clever_format([flops, params], "%.3f")
    # print('unet_plus flops:', macs)
    # print('unet_plus params:', params)
