...
def get_refinenet(input_size, num_classes, features=256, bn_momentum=0.01, pretrained=True):
    resnet101 = get_resnet101()      # new ResNet proposed in PSPNet
    return RefineNet4Cascade(input_size, num_classes=num_classes, resnet_factory=resnet101,
                             features=features, bn_momentum=bn_momentum, pretrained=pretrained)


def get_refinenet_model(n_class=1,image_size=512):
    model = get_refinenet(input_size=image_size, num_classes=n_class, pretrained=False)
    return model

def test():
    device = torch.device('cuda')  # cuda:0
    inputs = torch.rand(3, 256, 256).unsqueeze(0).to(device)
    print(inputs.shape)

    net = get_refinenet_model().to(device)
    res = net(inputs)
