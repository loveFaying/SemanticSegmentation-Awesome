...
class DeepLabV3plus(nn.Module):
    def __init__(self, class_num, bn_momentum=0.01):
        ...
        self.Resnet101 = get_resnet101(dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True)
...
def get_deeplabv3plus_model(n_class=1):
    model = DeepLabV3plus(class_num=n_class)
    return model
...
def test():
    ...
    net = get_deeplabv3plus_model().to(device)
    res = net(inputs)
